...
import 'package:jeeb_app/features/order/manage_order/data/repositories/manage_order_repository.dart';
...
class ManageOrderBloc extends Bloc<ManageOrderEvent, ManageOrderState> {
  ...
  Future<void> _onAcceptDelivery(
    ...
    final result = await _repository.acceptDelivery(event.id, event.deliveryTime);
    result.fold(
      (failure) => emit(ManageOrderError(message: failure.message)),
      (_) => emit(const ManageOrderSuccess(message: 'Order assigned successfully')),
    );
  }
  ...
  Future<void> _onConfirmPickup(
    ...
    result.fold(
      (failure) => emit(ManageOrderError(message: failure.message)),
      (_) => emit(const ManageOrderSuccess(message: 'Order picked up successfully')),
    );
  }
  ...
  Future<void> _onMarkAsDelivered(
    ...
    result.fold(
      (failure) => emit(ManageOrderError(message: failure.message)),
      (_) => emit(const ManageOrderSuccess(message: 'Order delivered successfully')),
    );
  }
  ...
  Future<void> _onRejectDelivery(
    ...
    result.fold(
      (failure) => emit(ManageOrderError(message: failure.message)),
      (_) => emit(const ManageOrderSuccess(message: 'Order released back to pool')),
    );
  }
}
