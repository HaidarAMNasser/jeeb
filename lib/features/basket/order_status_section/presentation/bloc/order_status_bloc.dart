...
class OrderStatusBloc extends Bloc<OrderStatusEvent, OrderStatusState> {
  ...
  void _startRealtimeListener() {
    ...
    _deliveryIdSub = _orderStatusRtdb.watchOrderDeliveryId(state.orderId).listen(
      (deliveryId) {
        if (isClosed) return;
  ...
  void _listenToDriverLocation(int? deliveryId) {
    ...
    _driverLocationSub?.cancel();
    _driverLocationSub = _orderStatusRtdb.watchDriverLiveLocation(deliveryId).listen(
      (location) {
