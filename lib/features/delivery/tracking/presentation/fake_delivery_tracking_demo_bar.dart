...
/// QA / demo: toggles simulated GPS → `POST /tracking/update-location` (Firebase routeHistory).
class FakeDeliveryTrackingDemoBar extends StatelessWidget {
  const FakeDeliveryTrackingDemoBar({super.key, this.order});

  final OrderEntity? order;

  Future<void> _onTap(BuildContext context, FakeDeliveryTrackingController fake, bool on) async {
    if (on) {
      fake.setSimulating(false);
      return;
    }

    fake.setSimulating(true);

    final o = order;
    if (o == null) return;

    final ok = await fake.pushImmediateMockLocation(o);
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          ok
              ? AppTranslation.deliveryFakeGpsSent
              : AppTranslation.deliveryFakeGpsSendFailed,
        ),
                ...
                child: Row(
                  children: [
                    ...
                    Switch.adaptive(
                      ...
                      onChanged: (v) => _onTap(context, fake, !v),
                    ),
                  ],
                ),
              ),
